...
namespace Cial::Common {
    ...
    void SourceFile::error(Result &r, const std::string &message, const SourceLocation &location) const {
        ...
        for(int32_t i = startLine; i < stopLine; i++) {
            ...
            if(sourceLine == nullptr)
                break;
            const auto sourceText = substring(sourceLine->begin, sourceLine->end);
            if(i == targetLine) {
                stream << Colorizer::colorize(fmt::format("{:>4d}| ", i + 1), fmt::color::violet)
                       << Colorizer::colorizeRange(sourceText, location.start().column, location.end().column,
                                                   fmt::color::yellow, fmt::color::blue)
                       << '\n'
                       << std::string(6 + location.start().column, ' ') << messageIndicator;
            } else {
                stream << Colorizer::colorize(fmt::format("{:>4d}| ", i + 1), fmt::color::violet) << sourceText;
            }
        ...
        r.error(fmt::format("({}@{}:{}) {}", _path.filename().string(), location.start().line + 1,
                            location.start().column + 1, message),
                location, stream.str());
    }
    ...
    size_t SourceFile::pos() const { return _index; }
    ...
    size_t SourceFile::length() const { return _buffer.size(); }
    ...
    void SourceFile::seek(const size_t index) { _index = index; }
    ...
    int32_t SourceFile::next(Result &r) {
        ...
        if(ch >= 0x80) {
            const auto cp = utf8Decode(reinterpret_cast<char *>(_buffer.data() + _index), _buffer.size() - _index);
